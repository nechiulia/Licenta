...
public class AddFeedbackDialog extends AppCompatDialogFragment {

    private EditText iet_message;
    private RatingBar ratingBar;
    private AlertDialog.Builder builder;
    private LayoutInflater inflater;
    private AddFeedbackDialogListner listener;
    ...
    public void createBuilder(){
        builder.setView(view)
                ...
                .setPositiveButton(R.string.add_announcement_positiveButton_hint, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        if(iet_message.getText().length() >4) {
                            String message = iet_message.getText().toString();
                            float rating = ratingBar.getRating();
                            listener.apply(message, rating);
                        }
                        else{
                            iet_message.setError("Introduceți un mesaj de cel puțin 4 litere.");
                        }
                    }
                });
    }
    ...
    private void initComponents(){
        ...
        view =inflater.inflate(R.layout.add_feedback_dialog,null);

        iet_message=view.findViewById(R.id.add_feedback_tv_message);
        ratingBar=view.findViewById(R.id.add_feedback_ratingbar);
    ...
    public void onAttach(Context context) {
        ...
        try {
            listener= (AddFeedbackDialogListner) context;
        } catch (ClassCastException e) {
            throw new ClassCastException(context.toString()+getString(R.string.addSportDialog_error_classCastException));
        }
    }

    public interface AddFeedbackDialogListner{
        void apply( String message, float rating);
    }
}
